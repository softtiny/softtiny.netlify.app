...
const TurboGears = ({data:{tree,datoCmsWork}}) =>{ 
    console.log(tree)
    return (
        <Layout>
            <div>
                    ...
                    <div className="sheet__body">
                        <ul className="sidebar__menu">
                            ...
                            {tree.edges.map(({node:{relativePath,}},idx)=>{
                                return (<li  key={idx} >
                                    <a href={`/TurboGears/${relativePath}`} target="_blank">{relativePath}</a>
                                </li>)
                            })}
                        </ul>
                    </div>
...
export const query = graphql`
  query TurboGearsQuery {
    tree: allFile(filter: { sourceInstanceName: { eq: "TurboGears" } }) {
        edges {
          node {
            relativePath
          }
    ...
    datoCmsWork(slug: {eq: "turbogears"}) {
        title
        excerpt
        descriptionNode {
            childMarkdownRemark {
